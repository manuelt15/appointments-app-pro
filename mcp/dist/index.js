...
server.tool('list_appointments', 'List appointments with optional filters', {
    ...
    while (results.length < maxResults) {
        ...
        if (params.room_id)
            qs.set('room_id', params.room_id);
        if (params.calendar_id)
            qs.set('calendar_id', params.calendar_id);
        if (params.start)
        ...
        const response = await apiCallWithMeta(`/api/appointments?${qs.toString()}`);
...
server.tool('update_appointment', 'Update or move an existing appointment', {
...
server.tool('delete_appointment', 'Cancel or permanently delete an appointment', {
    id: z.string().describe('Appointment UUID'),
...
server.tool('check_availability', 'Check whether an exact time interval is available for one calendar, employee, or room', {
...
// Tool: list_calendars
server.tool('list_calendars', 'List all calendars (employees and rooms) for the business', {}, async () => {
